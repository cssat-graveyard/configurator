...
namespace Framework
{
	public class ChartInfo : IConfigurationBase
	{
		...
		public ChartInfo()
			: this(ChartType.NoChartType, String.Empty, null, new BaseOptionSet(), new Label(), new List<int>(), AddInputClass.NoAddInputClass)
		{ }

		public ChartInfo(ChartType chartType, string chartId, int? maxSets, BaseOptionSet baseOptionSet, Label label, List<int> hideColumns, AddInputClass addInputClass)
		{
		...
		{
			return (ChartType == ChartType.NoChartType && String.IsNullOrEmpty(ChartId) && MaxSets == null && BaseOptionSet.IsEmpty && Label.IsEmpty &&
				HideColumns.Count == 0 && AddInputClass == AddInputClass.NoAddInputClass);
		}
		...
		public override bool Equals(object obj)
		{
			...
			else
			{
				...
				return chartTypeEqual && chartIdEqual && maxSetsEqual && baseOptionSetEqual && labelEqual && hideColumnsEqual && addInputClassEqual;
			}
		}

		public override int GetHashCode()
		{
			return ChartType.GetHashCode() ^ ChartId.GetHashCode() ^ MaxSets.GetHashCode() ^ BaseOptionSet.GetHashCode() ^ Label.GetHashCode() ^
				HideColumns.GetHashCode() ^ AddInputClass.GetHashCode();
		}
	}
}
